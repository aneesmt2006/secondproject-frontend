...
export default function VisitHistory() {
  const { data, isLoading, error, refresh } = useVisitHistory();
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center py-20 space-y-4 bg-rose-50/50 rounded-[2.5rem] border border-rose-100">
        <AlertCircle className="w-10 h-10 text-rose-500" />
        <div className="text-center">
          <p className="text-cocoa font-bold">Oops! Something went wrong</p>
          <p className="text-xs text-cocoa/60 mt-1">{error}</p>
        </div>
        <button 
          onClick={refresh}
          className="flex items-center gap-2 px-6 py-2 bg-rose-500 text-white rounded-xl font-bold text-xs shadow-lg shadow-rose-500/20 transition-all hover:scale-105 active:scale-95"
        >
        ...
        >
          ...
          <div className="relative flex flex-col sm:flex-row justify-between gap-6">
            <div className="space-y-4">
              ...
              <div>
                <h3 className="text-2xl font-bold tracking-tight">{upcoming.doctorName}</h3>
            ...
            <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-4 border border-white/10 flex flex-col items-center justify-center min-w-[120px]">
              ...
              <span className="text-3xl font-black">{upcoming.appoinmentTime.split(' ')[0]}</span>
              <span className="text-[10px] font-bold text-emerald-300 mt-1">{upcoming.appoinmentTime.split(' ')[1]}</span>
            </div>
          </div>
        </motion.div>
      )}
              ...
              <div className="flex md:flex-col items-center gap-3 shrink-0 relative">
                <div className="relative z-10 w-14 h-14 rounded-2xl bg-cream flex flex-col items-center justify-center text-wine shadow-inner border border-wine/5">
                  <span className="text-[10px] font-black uppercase tracking-tighter opacity-70">
                    {visit.appoinmentDate.split(',')[0].substring(0, 3)}
                  </span>
                  <span className="text-xl font-bold leading-none">
                    {visit.appoinmentDate.split(' ')[1].replace(',', '')}
                  </span>
                </div>
